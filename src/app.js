...
function renderFilters() {
  ...
  return `<div class="toolbar"><div class="controls"><label class="search">${icon('search')}<span class="sr-only">Sök recept på namn</span><input id="search" type="search" value="${esc(state.query)}" placeholder="Vad vill du laga?" autocomplete="off"></label>
    ...
    <div class="random-prompt"><p>Svårt att bestämma dig vad du ska laga?</p><button class="primary random-button" data-action="random">${icon('shuffle')}Slumpa recept</button></div></div>`;
}
...
document.addEventListener('input', event => {
  ...
  if (event.target.name === 'minutes') $('#auto-tag').textContent = Number(event.target.value) > 0 && Number(event.target.value) <= 30 ? '✓ Snabbt läggs till automatiskt.' : 'Snabbt läggs till automatiskt vid högst 30 minuter.';
});
