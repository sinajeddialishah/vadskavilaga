...
function renderSettings() {
  state.detail = null;
  app.innerHTML = chrome(`<div class="page-heading"><div><h1>Inställningar</h1></div></div>
    <section class="settings-card"><h2>Dina taggar</h2><div class="settings-tags">${state.tags.filter(tag => !AUTO_TAGS.includes(tag)).map(tag => `<span class="badge removable-tag">${esc(tag)}${tag === 'Snabbt' ? '' : `<button type="button" class="tag-remove" data-action="remove-tag" data-tag="${esc(tag)}" aria-label="Ta bort taggen ${esc(tag)}">${icon('close')}</button>`}</span>`).join('')}</div><form id="tag-form" class="tag-form"><label for="new-tag" class="sr-only">Ny tagg</label><input id="new-tag" name="tag" maxlength="30" required><button class="primary" type="submit">${icon('plus')}Lägg till</button></form><p id="tag-error" class="error inline-error" role="alert"></p></section>
